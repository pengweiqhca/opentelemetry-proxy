...
public class ActivityInvokerFactory : IActivityInvokerFactory, IDisposable
{
    ...
    private IActivityInvoker CreateActivityInvoker(IInvocation invocation, ActivityType activityType)
    {
        ...
        var settings = ActivityInvokerHelper.GetActivityName(invocation.Method, type, out var activityName, out var kind, out var maxUseableTimes);
        ...
        return settings switch
        {
            ...
            ActivitySettings.ActivityNameOnly => (IActivityInvoker)Activator.CreateInstance(ActivityInvokerHelper.GetActivityNameInvokerType(invocation.Method.ReturnType), activityName, maxUseableTimes, false),
            ActivitySettings.NonActivityAndSuppressInstrumentation => (IActivityInvoker)Activator.CreateInstance(ActivityInvokerHelper.GetActivityNameInvokerType(invocation.Method.ReturnType), activityName, maxUseableTimes, true),
