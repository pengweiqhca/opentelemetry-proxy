...
internal sealed class ObjectMethodExecutor
{
        ...
        CoercedAwaitableInfo coercedAwaitableInfo)
    {
        ...
        return Expression.Lambda<MethodExecutorAsync>(returnValueExpression, targetParameter, parametersParameter)
            .Compile();
    }

    public static Expression ConvertToObjectMethodExecutorAwaitable(CoercedAwaitableInfo coercedAwaitableInfo,
        Expression awaitable)
    {
        ...
        var getResultExpression = Expression.Call(Expression.Convert(getResultParam, awaitableInfo.AwaiterType),
            awaitableInfo.AwaiterGetResultMethod);

        var getResultFunc = Expression.Lambda<Func<object, object>>(
            awaitableInfo.AwaiterGetResultMethod.ReturnType == typeof(void)
