...
本设计将 OpenTelemetry.StaticProxy 从基于 Metalama `ISourceTransformer` 的编译期语法树重写方案，迁移到基于 Roslyn Incremental Source Generator + C# Interceptors 的方案。
...
namespace OpenTelemetry.StaticProxy.Generated
{
    // ActivitySource 实例集中管理
    ...
    {
        [System.Runtime.CompilerServices.InterceptsLocation(1, "...base64...")]
        internal static void MyMethod_Intercept_0(this MyNamespace.MyClass @this, int param)
        {
            var activity = ActivitySourceHolder.ActivitySource_MyNamespace_MyClass
                .StartActivity("MyNamespace.MyClass.MyMethod", default);
...
拦截方法必须与被拦截方法签名兼容：
- **实例方法**：拦截方法为 `static` 扩展方法，第一个参数为 `this T receiver`
...
internal static ReturnType MethodName_Intercept_N(this T @this, params...)
{
    ...
    if (activity != null)
    {
        activity.SetTag("tagName", paramValue);  // InTags
    }

    var disposable = SuppressInstrumentationScope.Begin();  // 仅当 SuppressInstrumentation=true
    try
    {
        var result = @this.OriginalMethod(args...);  // 调用原方法

        if (activity != null)
        {
            activity.SetTag("outTag", result);  // OutTags
        }

        return result;
    }
    catch (Exception ex) when (ActivityExtensions.SetExceptionStatus(activity, ex))
    {
        throw;
    }
    finally
    {
        disposable?.Dispose();  // 仅当 SuppressInstrumentation=true
        activity?.Dispose();
    }
}
```
...
<Project Sdk="Microsoft.NET.Sdk">
  ...
  <ItemGroup>
    ...
    <!-- 5.0.0 对应 .NET 10 SDK，确保 Interceptors 为正式特性且 GetInterceptableLocation API 可用 -->
  </ItemGroup>
